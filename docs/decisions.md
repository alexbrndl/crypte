...
**Why the fourth argument was optional, and why it is not any more.** The reason written here was that an adapter built against the previous shape keeps compiling. Measured with this repository's own `tsc --strict`: it keeps compiling either way. A three-parameter implementation is assignable to a four-parameter signature whatever the `?` says, because that is TypeScript's arity rule, and `packages/react/test/stories.test-d.ts` now pins it so the false premise cannot be rewritten. The `?` only ever served callers, and the one caller, the generated entry, has always passed all four. It is required since `DCJ-289`, so a caller that forgets the wrappers is told at the call site instead of silently mounting without them. The runtime default stays: the package is published, and a JavaScript caller can still leave the argument out.
...
**Why.** Three candidates of the first wave fell on doubt the refuters wrote down rather than argued away: two where neither could build the input, one whose premise was simply false. That wave put 22 candidates forward and removed 18. Across all four, counted on the diff of the published sources: nine exports, and every fallback or guard no input could reach.

**A refuter can be wrong, and one was.** The fourth candidate it stopped, `serve.ts shellHtml`, was given as a leftover and a refuter claimed to reach it "through a shell state rather than through a URL". Measured afterwards on a listening server, with a marker inside the function and eight shapes of route: it is reached by none of them. `sirv` is registered first and answers `/` and `/index.html` by its own `extensions` default, and it only ever declines when `dist/shell/index.html` is missing, which `shellAssets` refuses before the plugin exists. The function was dead, the refutation was false, and the rule kept dead code rather than losing live code. **That direction is the one to fail in**, but a refutation that names no measurement is worth no more than the reading it contradicts.
...
**What would reopen it.** A refuter that refutes nothing across a whole wave, which would mean it has become a rubber stamp rather than an opponent. A deletion this rule allowed and that had to be put back. Or the failure the `shellHtml` case showed, if it repeats: a refutation asserted from reading, with no command behind it. The cure for that one is already written elsewhere in this repository, "un doute se tranche par une commande" — it binds the refuter as much as the reader, and the rule above does not say so.
