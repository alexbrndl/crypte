...
*Seize formes croisées*, dont `typeof valeur` dans un argument de type, écarté à raison puisqu'un nom effacé à l'exécution n'a rien à pré-empaqueter, et `<Q>valeur`, qui garde sa valeur par la seule exception qui compte. Dix cas d'`adapter.test.ts` les tiennent : le garde retiré, dix rougissent ; l'ancien tri par clé remis seul, un rougit, celui de `<T extends Q>`.
