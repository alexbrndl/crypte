...
*La liste des nœuds à valeur n'est pas close*, et c'est le seul endroit où l'oubli d'un nom retire un import. Douze noms mesurés : les cinq expressions qui portent leur valeur sous `expression`, `TSParameterProperty` sous `parameter`, un membre d'énumération sous `initializer`, un `namespace` par son bloc, `TSImportEqualsDeclaration` par sa référence de module, et `TSQualifiedName`, qui n'est atteignable que depuis cette déclaration puisque ses deux parents du côté type, `TSTypeReference` et `TSImportType`, sont déjà sautés. Sa droite est exclue comme celle d'un accès membre : `import E = fait.Named` ne doit pas faire voyager `@acme/named`.
...
*Vingt-trois formes croisées*, dont `typeof fait` dans un argument de type, écarté à raison puisqu'un nom effacé à l'exécution n'a rien à pré-empaqueter, et `<Q>fait`, qui garde sa valeur par l'exception qui compte. Vingt et un cas d'`adapter.test.ts` les tiennent, et chaque pièce est surveillée séparément, mesuré : les six noms de déclaration retirés de la liste font rougir quatre cas, le `continue` sur `decorators` un, l'exclusion de la droite d'un nom qualifié un, le garde par nœud du tour 3 dix.
