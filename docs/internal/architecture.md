...
## 4 quater. La mesure du poids d'un manifeste
...
## 4 quinquies. Le code publié en anglais

`test/published-english.test.mjs` refuse un caractère accentué dans `packages/*/src`, hors de ce qui est cité entre accents graves.

**Pourquoi.** L'anglais n'en porte aucun, donc une seule ligne revenue au français se voit. Ce qu'un utilisateur ou un contributeur lit est en anglais depuis le lot 0 quindecies ; les notes de conception restent en français, dans `docs/internal/`. Voir `docs/decisions.md`.
...
*Le garde-fou posé pour ce couplage n'a pas fonctionné, et c'est la leçon la plus utile du lot.* Un cas affirmait `PREVIEW_ENTRY.endsWith('.js')`, pour rougir le jour où l'entrée passerait en `.ts`. Or `DCJ-224` s'est corrigé **sans changer le chemin** : l'entrée est compilée par le plugin, et le chemin reste juste. Le cas est donc resté vert pendant que sa prémisse disparaissait. Il affirmait un **symptôme attendu** de la décision, pas la décision : un garde-fou doit affirmer la chose dont il dépend, ici que l'entrée servie n'est pas compilée, et cette chose-là n'était pas exprimable en une ligne. Le cas est retiré, et les six noms sont revenus.
...
**Ce qu'un membre d'interface porte, et où.** Les trois formes qu'oxc range sous `TSMethodSignature` ne se lisent pas au même endroit : un getter porte son type dans `returnType`, un setter dans son paramètre, et une méthode n'en a pas à lire, c'est une fonction. Mesuré. Les marquer sur le type du nœud plutôt que sur son `kind` faisait sortir `get tone(): string` en `function`, et les lire comme des propriétés les faisait disparaître, le motif les rattrapant alors en facultatif alors que le type les déclare requis. Une omission se voit, un drapeau faux se croit.
...
| ce qui fait foi : contrats, décisions, suivi, `CLAUDE.md`, `.claude/**` | modèle courant |
...
**Cinq formes valent du code malgré leur extension.** Les trois premières font foi, `suivi.md` portant les défauts arbitrés avec leur mesure. `CLAUDE.md` et les skills portent les règles de travail, donc une erreur dedans se propage à toutes les sessions suivantes. La liste exacte vit dans `test/review-check.mjs`, pas ici, pour qu'un test la garde.
...
**`CLAUDE.md` disait le contraire, et il a été amendé dans le même diff.** Sa règle de flux portait « brouillon, revue, puis ouverture, dans cet ordre, sans exception », donc le workflow ne remplissait pas un silence : il appliquait la règle. L'exception y est maintenant écrite, avec la même liste.

À ne pas confondre avec sa règle voisine, « s'en passer s'ils ne touchent que de la documentation », qui porte sur le fait de **relancer** un tour après du code ajouté post-revue, pas sur le premier.
...
*Ce qui casse si on l'enlève :* le classement dérive sans qu'aucune assertion ne bouge. Mesuré avant l'extraction : un `CLAUDE.md` imbriqué dans un paquet, et un fichier de contrats scindé en dossier, se classaient tous deux comme de la prose. Ni l'une ni l'autre de ces formes n'existe aujourd'hui, et c'est précisément pour ça que personne ne l'aurait vu.
