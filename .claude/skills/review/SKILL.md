...
1. **Les quatre contraintes structurelles de `CLAUDE.md`.** Une dépendance interne embarquée en copie, un composant placé dans `core`, ou dans `@crypte/ui` avec une prop qu'un seul consommateur utilise, un import de `vite-plus` dans du code publié, une entrée de `core` qui en tire une autre.
