...
- [#83](https://github.com/alexbrndl/crypte/pull/83) [`ddb4f45`](https://github.com/alexbrndl/crypte/commit/ddb4f45a5dea300d852d1cacddc0e7d65758914f) Thanks [@alexbrndl](https://github.com/alexbrndl)! - Everything that runs in the shell is named `shell`: the `@crypte/core/shell` entry, and a plugin's `shell` surface, typed `ShellContribution`.
...
- [#66](https://github.com/alexbrndl/crypte/pull/66) [`a076311`](https://github.com/alexbrndl/crypte/commit/a076311250a9aca656a2df76368d9207a419bd3e) Thanks [@alexbrndl](https://github.com/alexbrndl)! - `ShellMessage` no longer carries `update-overrides` or `set-globals`. Neither had any effect, and `render` already applies the overrides it is given. Both are in section 7 of the contracts, with what would bring them back.
...
- [#90](https://github.com/alexbrndl/crypte/pull/90) [`0271aac`](https://github.com/alexbrndl/crypte/commit/0271aaccf07aac59919c2160178d05bf28a3b75c) Thanks [@alexbrndl](https://github.com/alexbrndl)! - No change to the published code: the `sideEffects: false` the core declares is now checked by its own tests, which read every source file and fail on anything that runs at import.
...
- [#12](https://github.com/alexbrndl/crypte/pull/12) [`004c342`](https://github.com/alexbrndl/crypte/commit/004c3427ce3580377c2a5a228056c3b2a5a91379) Thanks [@alexbrndl](https://github.com/alexbrndl)! - Ajoute le canal `postMessage` entre le shell et la preview, et le montage React.
  ...
  `@crypte/core/shell` expose `createShellChannel`, `@crypte/core/preview` expose `createPreviewChannel`, et `@crypte/react` expose `createAdapter` pour monter un composant dans la preview. Le shell ne manipule que des messages sérialisables et ne charge aucun framework de rendu.
