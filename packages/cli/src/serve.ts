...
export function previewEntry(project: Project, files: string[] = []): string {
  ...
  return [
    ...
    `function ${OWN}named(failure, loading) {`,
    ...
    '  if (missing && loading) {',
    `    const module = missing[1].split('?')[0].replace(/^\\/@fs/, '').replace(${JSON.stringify(`${project.root}/`)}, '').replace(/^\\//, '')`,
    ...
    `  if (${OWN}failure) throw ${OWN}named(${OWN}failure, entry.storyFile)`,
    '',
    ...
    `  for (const [path, failed] of ${OWN}stale) throw ${OWN}named(failed, path.slice(1))`,
    '',
