...
export async function loadProject(root: string): Promise<Project> {
  ...
  // et rend les dépendances à surveiller.
  const loaded = await loadConfigFromFile({ command: 'serve', mode: 'development' }, file, root)
  if (!loaded) {
...
export function cssEntryOf(project: Project): string | undefined {
  ...
  return isAbsolute(css) ? css : join(project.root, css)
}
